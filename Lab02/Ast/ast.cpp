...
Logical::Logical(Token *t, Expression *e1, Expression *e2) : Expression(NodeType::LOG, ExprType::BOOL, t), expr1(e1), expr2(e2)
{
    ...
    if (expr1->type != ExprType::BOOL || expr2->type != ExprType::BOOL)
    {
        if (expr1->type == ExprType::INT || expr2->type == ExprType::INT)
        {
            ...
            {
                int int_value1 = std::stoi(expr1->token->lexeme);
            ...
            {
                int int_value2 = std::stoi(expr2->token->lexeme);
...
UnaryExpr::UnaryExpr(int etype, Token *t, Expression *e) : Expression(NodeType::UNARY, etype, t), expr(e)
{
    ...
    {
        int int_value1 = std::stoi(expr->token->lexeme);
        bool bool_value1 = (int_value1 != 0);
        Token *bool_token1 = new Token(*expr->token);
        bool_token1->lexeme = bool_value1 ? "true" : "false";
        expr = new Expression(NodeType::LOG, ExprType::BOOL, bool_token1);
    }
    // verificação de tipos
